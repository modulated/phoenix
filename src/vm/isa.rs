...
impl<'a> Cpu<'a> {
    ...
    fn suba(&mut self, reg: u16, ea: AddressingMode, size: Size) {
        println!("SUBA {size:?} An:{reg} EA:{ea:?}");
        todo!()
    }

    fn sub_dn(&mut self, reg: u16, ea: AddressingMode, size: Size) {
        println!("SUB {size:?} Dn:{reg} EA:{ea:?}");
        todo!()
    }

    fn sub_ea(&mut self, reg: u16, ea: AddressingMode, size: Size) {
